<template>
  <div class="fixed bottom-0 left-0 right-0 flex flex-col border-t-2 border-slate-100 p-4 bg-white shadow-lg shadow-slate-100">
    <div class="grid grid-cols-3 gap-3 mb-2">
      ...
      <button :class="`px-2 py-1 border-2 border-slate-300 rounded-lg text-center ${activeTabs === 'pdf' ? 'border-sky-300 text-sky-300' : 'text-slate-300'}`" @click="activeTabs = 'pdf'">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
export default {
  ...
  methods: {
    ...
    onSendPDF() {
      if (!!this.pdfFile) this.$emit('onSendPdf', this.pdfFile);
      this.pdfFile = '';
    },
  }
}
